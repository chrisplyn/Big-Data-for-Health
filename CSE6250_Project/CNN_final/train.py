...
output_loc = "./output/" + model_name + " " + datetime.now().strftime('%Y-%m-%d')
...
# use 0.001 for DenseNet and 0.0001 for VGG16.
if model_name == "VGG16":
    initial_LR = 0.0001 
...
os.makedirs(output_loc) # create output folder
###################################################################################################
...
if model_name == "VGG16":
    base_model = VGG16(include_top=False, weights="imagenet", pooling="avg") # Compare 'max' and 'avg' pooling, 'avg' works much better.
if model_name == "DenseNet121":
